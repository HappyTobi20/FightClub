...
{
    return Rectangle(m_x, m_y, c_config.width, c_config.height);
}

Rectangle Player::sweapAABB() const
{
    const float startY = std::min(m_previousY, m_y);
    const float endY = std::max(m_previousY, m_y);

    return Rectangle(m_x, startY, c_config.width, endY - startY + c_config.height);
}
