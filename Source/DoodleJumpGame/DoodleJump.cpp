...
void DoodleJump::update() 
{
    ...
    if (m_player->velocityY() > 0.0f)
    {
        ...
        {
            if (CheckCollisionRecs(m_player->aabb(), platform))
            {
                m_player->jump();
                break;
            }
...
void DoodleJump::draw() 
{
    ...
    DrawRectangleRec(m_player->sweapAABB(), {255, 255, 255, 120});
...
{
    const int gameOverTextWidth = MeasureText("GAME OVER", 60);
    ...
    BeginDrawing();
    ClearBackground(BLACK);
    DrawText("GAME OVER", gameOverCenterX, c_screenSize.height / 2 - 80, 60, WHITE);
